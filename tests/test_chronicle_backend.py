...
def chronicle_custom_backend():
    return ChronicleBackend(query_settings = lambda x: {"custom.query.key": x.title}, output_settings = {"custom.key": "customvalue"})
...
def test_chronicle_and_or_expression(chronicle_backend : ChronicleBackend):
    rule = SigmaCollection.from_yaml("""
            ...
            detection:
                ...
                condition: sel
        """)
    assert chronicle_backend.convert(rule) == ['(fieldA="valueA1" OR fieldA="valueA2") AND (fieldB="valueB1" OR fieldB="valueB2")']
...
def test_chronicle_or_and_expression(chronicle_backend : ChronicleBackend):
    rule = SigmaCollection.from_yaml("""
            ...
            detection:
                ...
                condition: 1 of sel*
        """)
    assert chronicle_backend.convert(rule) == ['(fieldA="valueA1" AND fieldB="valueB1") OR (fieldA="valueA2" AND fieldB="valueB2")']
